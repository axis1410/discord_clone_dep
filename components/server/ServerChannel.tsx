...
import { ActionTooltip } from "../ActionnTooltip";
...
export const ServerChannel: FC<ServerChannelProps> = ({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        `group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-zinc-700/10 
      ...
      {channel.name !== "general" && role !== MemberRole.GUEST && (
        <div className="ml-auto flex items-center gap-x-2">
          <ActionTooltip label="Edit Channel" side="top">
            <Edit
              onClick={(e: MouseEvent) => onAction(e, "editChannel")}
          ...
          <ActionTooltip label="Delete Channel" side="top">
            <Trash
              onClick={(e: MouseEvent) => onAction(e, "deleteChannel")}
              className="hidden group-hover:block w-4 h-4 text-zinc-500 hover:text-zinc-600 
            dark:text-zinc-400 dark:hover:text-zinc-300 transition"
            />
          </ActionTooltip>
